...
def vmap(
  ...
  *,
  ...
  Example::
    ...
    ... def initialize_ensemble(rngs):
    ...   return Model(2, 3, rngs=rngs)
    ...
    >>> rngs = nnx.Rngs(params=0, dropout=1)
    >>> nnx.split_rngs(rngs, 5)
    [...]
    >>> ensemble = initialize_ensemble(rngs)
    >>> ensemble.linear.kernel.shape
    (5, 2, 3)
    >>> ensemble.bn.scale.shape
    (5, 3)
  ...
  batch statistics and dropout random state::
    ...
    ...  (nnx.BatchStat, 'dropout'): 0,
    ...  (nnx.Param, 'params'): None,
    ... })
    ...
    ...
    ...
    >>> rngs = nnx.Rngs(params=0, dropout=1)
    >>> nnx.split_rngs(rngs, 5, filter='dropout')
    [...]
    >>> ensemble = initialize_ensemble(rngs)
    >>> ensemble.linear.kernel.shape
    (2, 3)
    >>> ensemble.bn.mean.shape
    (5, 3)
    >>> @nnx.experimental.vmap(in_axes=(state_axes, None), out_axes=0)
    ... def forward(model, x):
    ...   return model(x)
    ...
    >>> x = jnp.ones((4, 2))
  ...
  To share the same dropout masks between ensemble members::

    >>> state_axes = nnx.StateAxes({
    ...  nnx.BatchStat: 0,
    ...  (nnx.Param, 'params', 'dropout'): None,
    ... })
    ...
    >>> @nnx.experimental.vmap(in_axes=(state_axes,), out_axes=state_axes, axis_size=5)
    ... def initialize_ensemble(rngs):
    ...   return Model(2, 3, rngs=rngs)
    ...
    >>> rngs = nnx.Rngs(params=0, dropout=1)
    >>> # << no need to split rngs here
    >>> ensemble = initialize_ensemble(rngs)
    >>> ensemble.linear.kernel.shape
    (2, 3)
    >>> ensemble.bn.mean.shape
